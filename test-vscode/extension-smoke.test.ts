...
suite('Quick JSON Viewer VS Code smoke tests', () => {
  ...
  test('opens staged Git JSON diffs with VS Code text diff editor', async function () {
    ...
    const repository = await openGitRepository(repoDir);
    await fs.writeFile(fileUri.fsPath, '{"a":2}\n', 'utf8');
    await runGit(repoDir, ['add', 'fixture.json']);
...
async function createGitJsonFixture(): Promise<{
  ...
  await runGit(repoDir, ['config', 'user.email', 'quick-json-viewer@test']);
  await runGit(repoDir, ['config', 'user.name', 'Quick JSON Viewer Test']);
  await fs.writeFile(filePath, '{"a":1}\n', 'utf8');
  await runGit(repoDir, ['add', 'fixture.json']);
  await runGit(repoDir, ['commit', '-m', 'Initial JSON fixture']);
