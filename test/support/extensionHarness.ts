  ...
  public readonly vscode = {
    ...
    workspace: {
      getConfiguration: (section: string) => {
        assert.equal(section, 'quickJsonViewer');
        return {
          get: (key: string): unknown => {
            if (key === 'largeFileThresholdMb') {
              return this.largeFileThresholdMb;
            }
          ...
          update: async (
            ...
            if (key === 'largeFileThresholdMb' && typeof value === 'number') {
              this.largeFileThresholdMb = value;
            }
...
export function loadExtension(
  ...
  const realJson = require('../../src/json') as Record<string, unknown>;
